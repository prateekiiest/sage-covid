...
chronic_conditions_concept_ids = [137809, 380055, 135777, 196360, 439392, 320136, 257004, 260139, 40480893, 4110056, 4079750, 4079749, 80180, 80809, 40483287, 193782, 201620, 443611, 443597, 46271022, 443614, 443601, 443612, 316139, 319835, 4229440, 4158911, 315286, 319844, 40481919, 4127089, 46273649, 764123, 36712779, 37312532, 46270162, 46270163, 4152384, 4110961, 373503, 321319, 320746, 45757557, 4163710, 22281, 321263, 443738, 42538119, 42539502, 42538117, 42539698, 201254, 443412, 201826, 443731, 376065, 443729, 37018196, 45770830, 4042728, 443733, 4214376, 192680, 312902, 317895, 319826, 320128, 320128, 443771, 4013643, 4110948, 4118910, 4120094, 4167493, 4313767, 4322024, 4339214, 43020910, 44782715,  44783628, 192855, 37396808, 4271013, 313236, 4138760, 4146581, 4279553, 45768910, 45768963, 45768964, 45768965, 37116845, 45769350, 45769351, 45769352, 45769438, 45771045, 46270082, 46273487, 381591, 193174, 254320, 441267, 440371, 200762, 441269, 4146936, 4223448, 4281109, 45765493, 436659, 45768812, 4146209, 439777, 137820, 257628, 4031164, 4055224, 4058694, 4058695, 4064161, 4001171, 198964, 439674, 197494, 253797, 45763750, 30978, 4278669, 4287844, 4141360, 313217, 4154290, 4232697, 257007, 259848, 260123, 257012, 443454, 321318, 435243, 433753, 4188598, 4132434, 4209423, 440360, 440417, 4327889, 40481089]
...
def prepare_feature_matrix_measurement(person_ids, person_indices, covid_measurement, measurement, measurement_feature_indices, X_min, X_max, X_ave):
		...
		measurement_date_dt = dt.datetime.strptime(measurement_date, "%Y-%m-%d")
                starting_date = (measurement_date_dt - dt.timedelta(days=60)).strftime('%Y-%m-%d')
		ending_date = (measurement_date_dt + dt.timedelta(days=60)).strftime('%Y-%m-%d')

                subm_after_covid = measurement.query('measurement_date >= @starting_date and measurement_date <= @ending_date and person_id == @person_id')
                subm_before_covid = measurement.query('(measurement_date < @starting_date or measurement_date > @ending_date) and person_id == @person_id')
...
def prepare_feature_matrix_condition(covid_measurement, df, feature_indices, query_string, query_string_2, query_string_3, X_table):
		...
		if (int(i) in chronic_conditions_concept_ids):
			chronic_condition_flag = 1
		else:
			chronic_condition_flag = 0
 
                person_ids = subm.drop_duplicates('person_id')['person_id'].tolist()

                for j in range(len(person_ids)):

                        person_id = person_ids[j]

			if (chronic_condition_flag == 1):
				subm_after_covid = subm.query(query_string_2)
				...
				subm_after_covid = subm.query(query_string_3 + ' >= @starting_date and ' + query_string_3 + ' <= @ending_date and person_id == @person_id')

                        if (subm_after_covid.empty):
                                continue

                        index_p = person_indices[person_id]
                        X_table[index_p][index_f] += len(subm_after_covid)

        return X_table

def prepare_feature_matrix_count_based(person_ids, person_indices, covid_measurement, df, query_string, concept_id_str, feature_indices, X):
        ...
        for person_id in person_ids:
                ...
                subm_after_covid = df.query(query_string + ' >= @starting_date and ' + query_string + ' <= @ending_date and person_id == @person_id')
...
def prepare_feature_matrix_drug(covid_measurement, df, feature_indices, query_string, query_string_2, X_table):
        ...
        for i in concept_ids:
                ...
                for j in range(len(person_ids)):
	                ...
	                subm_after_covid = subm.query(query_string_2 + ' >= @starting_date and ' + query_string_2 + ' <= @ending_date and person_id == @person_id')

			if (subm_after_covid.empty):
				continue

			quantities = subm_after_covid['quantity'].to_numpy()
...
drug_exposure = pd.read_csv('../../ehr-dream-challenges/examples/covid19-question-2/synthetic_data/training/drug_exposure.csv',usecols = ['drug_concept_id', 'person_id', 'drug_exposure_start_date', 'drug_exposure_end_date', 'quantity'])
...
df = condition
query_string = 'condition_concept_id in @i'
query_string_2 = 'person_id == @person_id'
query_string_3 = 'condition_start_date'
feature_indices = condition_feature_indices
X = X_condition 
X_condition = prepare_feature_matrix_condition(covid_measurement, df, feature_indices, query_string, query_string_2, query_string_3, X)
...
df = device_exposure
query_string = 'device_exposure_start_date'
concept_id_str = 'device_concept_id'
feature_indices = device_exposure_feature_indices
X = X_device_exposure
X_device_exposure = prepare_feature_matrix_count_based(person_ids, person_indices, covid_measurement, df, query_string, concept_id_str, feature_indices, X)

print(datetime.now())
print("drug exposures")

df = drug_exposure
query_string = 'drug_concept_id in @i'
query_string_2 = 'drug_exposure_start_date'
X_drug_exposure = prepare_feature_matrix_drug(covid_measurement, df, drug_exposure_feature_indices, query_string, query_string_2, X_drug_exposure)

print(datetime.now())
print("procedures")

df = procedure
query_string = 'procedure_date'
concept_id_str = 'procedure_concept_id'
feature_indices = procedure_feature_indices
X = X_procedure
X_procedure = prepare_feature_matrix_count_based(person_ids, person_indices, covid_measurement, df, query_string, concept_id_str, feature_indices, X)

print(datetime.now())
print("visits")

df = visit
query_string = 'visit_start_date'
concept_id_str = 'visit_concept_id'
feature_indices = visit_feature_indices
X = X_visit
X_visit = prepare_feature_matrix_count_based(person_ids, person_indices, covid_measurement, df, query_string, concept_id_str, feature_indices, X)
...
#selector = SelectFromModel(estimator=LogisticRegression(solver='saga', n_jobs=-1)).fit(X_selected, Y)
