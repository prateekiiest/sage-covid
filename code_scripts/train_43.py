...
print(datetime.now())
print("Measurement Count Features")

df = measurement
query_string = 'measurement_date'
concept_id_str = 'measurement_concept_id'
feature_indices = measurement_feature_indices
X = X_measurement_counts
X_measurement_counts = prepare_feature_matrix_count_based(person_ids, person_indices, covid_measurement, df, query_string, concept_id_str, feature_indices, X)

print(datetime.now())
print("conditions")

df = condition
query_string = 'condition_start_date'
concept_id_str = 'condition_concept_id'
feature_indices = condition_feature_indices
X = X_condition 
X_condition = prepare_feature_matrix_count_based(person_ids, person_indices, covid_measurement, df, query_string, concept_id_str, feature_indices, X)
...
parameters = { 'learning_rate_init': [0.001, 0.01, 0.1],
        ...
        'hidden_layer_sizes': [(20,), (25,), (30,), (35,), (40,), (45,), (50,)],
        'alpha': [0.0, 0.0001, 0.001, 0.01, 0.1, 1.0, 10.0],
        'batch_size': [50, 100, 150, 200],
	'validation_fraction': [0.2, 0.4],
        'early_stopping': [True, False]
}

clf = GridSearchCV(mlp, parameters, n_jobs=-1, verbose=1, scoring=auprc_score, cv=3)

search = clf.fit(X_selected, Y)
best_params = search.best_params_
learning_rate_init_opt = best_params['learning_rate_init']
hidden_layer_sizes_opt = best_params['hidden_layer_sizes']
alpha_opt = best_params['alpha']
batch_size_opt = best_params['batch_size']
early_stopping_opt = best_params['early_stopping']

print(datetime.now())
print("Model training")

mlp = MLPClassifier(solver='adam', learning_rate_init=learning_rate_init_opt, hidden_layer_sizes=hidden_layer_sizes_opt, alpha=alpha_opt, batch_size=batch_size_opt, early_stopping=early_stopping_opt).fit(X_selected, Y)

#clf = LogisticRegression(solver='saga', n_jobs=-1).fit(X_selected, Y)
#clf = MLPClassifier(learning_rate_init=0.01, hidden_layer_sizes=25, early_stopping=True, alpha=0.001).fit(X_selected, Y)
#clf = lgb.LGBMClassifier(n_jobs = 4).fit(X_selected, Y)

print(datetime.now())

dump(mlp, '../../model/baseline.joblib')
