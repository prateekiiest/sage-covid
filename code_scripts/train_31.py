...
parameters = {'C': loguniform(math.pow(2,-5), math.pow(2,5))}

clf = RandomizedSearchCV(lr, parameters, random_state=0, n_iter=500, n_jobs=-1, scoring=auprc_score, cv=3, verbose=1)
...
clf = LogisticRegression(solver='saga', C=C_opt, n_jobs=-1).fit(X_selected, Y)
