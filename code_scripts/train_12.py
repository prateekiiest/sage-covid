...
def add_COVID_measurement_date():
    ...
    measurement = measurement.sort_values(['measurement_date'],ascending=True).groupby('person_id').head(1)
...
X_selected = np.concatenate((X_procedure_selected, X_age), axis=1)
...
clf = RandomizedSearchCV(svm, parameters, random_state=0, n_iter=500, n_jobs=4, scoring=auprc_score, cv=3, verbose=2)
...
clf = SVC(C=C_opt, gamma=gamma_opt, probability=True).fit(X_selected, Y)
