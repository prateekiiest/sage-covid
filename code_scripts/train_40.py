...
def feature_selection_measurement_auprc(X, Y):
        ...
        for i in range(n_iterations):
                ...
                y_pred = cross_val_predict(clf, X_opt_selected, Y_opt, cv=5, method='predict_proba')
...
def report_features(selected_feature_indices, cumulative_sums_best_n_features_selected_list, concept_ids_selected_first_step, feature_importances, one_hot_encoder_categories, feature_reporting_filename):
        ...
        ofp.write('\t\t"id": 1,\n')
        ofp.write('\t\t"name": "Most Recent RT-PCR Measurement",\n')
        ofp.write('\t\t"reference_column": "measurement_date",\n')
        ofp.write('\t\t"aggregator function": max,\n')
        ofp.write('\t\t"date": None\n')
        ofp.write('\t}],\n')
        ofp.write('\t"TimeRanges": [{\n')
        ofp.write('\t\t"id": 2,\n')
        ofp.write('\t\t"name": "60 days prior to latest rt-pcr measurement",\n')
        ofp.write('\t\t"date_index_id": 1,\n')
        ofp.write('\t\t"time_unit": "days",\n')
        ofp.write('\t\t"time_range": -60\n')
        ofp.write('\t},\n')
        ofp.write('\t{\n')
        ofp.write('\t\t"id": 3,\n')
        ofp.write('\t\t"name": "60 days after latest rt-pcr measurement",\n')
        ofp.write('\t\t"date_index_id": 1,\n')
        ofp.write('\t\t"time_unit": "days",\n')
        ofp.write('\t\t"time_range": +60\n')
        ofp.write('\t}],\n')
