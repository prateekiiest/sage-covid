...
X_count = np.zeros((len(person_index), n_measurement_types))
X_min[:] = -10
X_max[:] = -10
X_ave[:] = -10
...
for i in measurement_feature_concept_ids:
        ...
        subm_after_covid_count = subm_after_covid.groupby('person_id')['value_as_number'].count()
        ...
        for person_id in subm_after_covid_min.keys():
                ...
                X_count[index_p][index_f] = subm_after_covid_count[person_id]
...
X_count_selected = X_count[:, 0:best_n_features_selected_measurement]
...
X_measurement_selected = np.concatenate((X_measurement_selected, X_count_selected), axis=1)
...
mean_zero = load('../model/mean_zero')
cov_zero = load('../model/cov_zero')
mean_one = load('../model/mean_one')
cov_one = load('../model/cov_one')
class_priors = load('../model/class_priors')

n_features = X_selected.shape[1]
cov_zero = cov_zero + 0.01*np.identity(n_features)
cov_one = cov_one + 0.01*np.identity(n_features)
...
for i in range(len(person_ids)):
	...
	if (multi_variate_normal_zero <= 1e-300):
		log_class_conditional_zero = -1e10
	else:
		log_class_conditional_zero = math.log(multi_variate_normal_zero)

	multi_variate_normal_one = multivariate_normal.pdf(X_selected_i, mean_one, cov_one)
        if (multi_variate_normal_one <= 1e-300):
                log_class_conditional_one = -1e10
        else:
                log_class_conditional_one = math.log(multi_variate_normal_one)
	

	log_x_joint_class_zero = log_class_conditional_zero + math.log(class_priors[0])
        log_x_joint_class_one = log_class_conditional_one + math.log(class_priors[1])
