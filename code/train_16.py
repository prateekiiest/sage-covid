...
if (X_measurement.shape[1] > 0):
	selector = SelectFromModel(estimator=ExtraTreesClassifier(n_estimators=50)).fit(X_measurement, Y)
	X_measurement_selected = selector.transform(X_measurement)
	selectorfile = '../model/selector_measurement.sav'
	dump(selector, open(selectorfile, 'wb'))
else:
	X_measurement_selected = X_measurement

if (X_condition.shape[1] > 0):
	selector = SelectFromModel(estimator=ExtraTreesClassifier(n_estimators=50)).fit(X_condition, Y)
	X_condition_selected = selector.transform(X_condition)
	selectorfile = '../model/selector_condition.sav'
	dump(selector, open(selectorfile, 'wb'))
else:
	X_condition_selected = X_condition

if (X_observation.shape[1] > 0):
	selector = SelectFromModel(estimator=ExtraTreesClassifier(n_estimators=50)).fit(X_observation, Y)
	X_observation_selected = selector.transform(X_observation)
	selectorfile = '../model/selector_observation.sav'
	dump(selector, open(selectorfile, 'wb'))
else:
	X_observation_selected = X_observation

if (X_device_exposure.shape[1] > 0):
	selector = SelectFromModel(estimator=ExtraTreesClassifier(n_estimators=50)).fit(X_device_exposure, Y)
	X_device_exposure_selected = selector.transform(X_device_exposure)
	selectorfile = '../model/selector_device_exposure.sav'
	dump(selector, open(selectorfile, 'wb'))
else:
	X_device_exposure_selected = X_device_exposure

if (X_drug_exposure.shape[1] > 0):
	selector = SelectFromModel(estimator=ExtraTreesClassifier(n_estimators=50)).fit(X_drug_exposure, Y)
	X_drug_exposure_selected = selector.transform(X_drug_exposure)
	selectorfile = '../model/selector_drug_exposure.sav'
	dump(selector, open(selectorfile, 'wb'))
else:
	X_drug_exposure_selected = X_drug_exposure

if (X_procedure.shape[1] > 0):
	selector = SelectFromModel(estimator=ExtraTreesClassifier(n_estimators=50)).fit(X_procedure, Y)
	X_procedure_selected = selector.transform(X_procedure)
	selectorfile = '../model/selector_procedure.sav'
	dump(selector, open(selectorfile, 'wb'))
else:
	X_procedure_selected = X_procedure

if (X_visit.shape[1] > 0):
	selector = SelectFromModel(estimator=ExtraTreesClassifier(n_estimators=50)).fit(X_visit, Y)
	X_visit_selected = selector.transform(X_visit)
	selectorfile = '../model/selector_visit.sav'
	dump(selector, open(selectorfile, 'wb'))
else:
	X_visit_selected = X_visit
