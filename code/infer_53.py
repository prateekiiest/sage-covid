...
kde_zero = load('../model/conditional_density_zero')
kde_one = load('../model/conditional_density_one')
class_priors = load('../model/class_priors')

log_class_conditional_zero = kde_zero.score_samples(X_selected)
log_class_conditional_one = kde_one.score_samples(X_selected)
...
numerator = np.exp(log_x_joint_class_one)
numerator_complement = np.exp(log_x_joint_class_zero)
denominator = numerator + numerator_complement
denominator[denominator == 0.0] = 1e-3
Y_pred = (numerator / denominator).reshape(n_persons, 1)

person_ids = person[['person_id']]
output = pd.DataFrame(Y_pred,columns = ['score'])
output_prob = pd.concat([person_ids,output],axis = 1)
