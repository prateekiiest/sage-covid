...
measurement_positives_after_covid = measurement.query('measurement_date > "2020-03-01" and person_id in @person_ids_positives')
...
condition_positives_after_covid = condition.query('condition_end_date > "2020-03-01" and person_id in @person_ids_positives')
...
device_exposure_positives_after_covid = device_exposure.query('device_exposure_end_date > "2020-03-01" and person_id in @person_ids_positives')
...
drug_exposure_positives_after_covid = drug_exposure.query('drug_exposure_end_date > "2020-03-01" and person_id in @person_ids_positives')
...
procedure_positives_after_covid = procedure.query('procedure_date > "2020-03-01" and person_id in @person_ids_positives')
...
visit_positives_after_covid = visit.query('visit_end_date > "2020-03-01" and person_id in @person_ids_positives')
...
for i in measurement_feature_concept_ids:
        ...
        subm_after_covid = subm.query('measurement_date > "2020-03-01"')
        subm_before_covid = subm.query('measurement_date <= "2020-03-01"')
...
for i in condition_feature_concept_ids:
        ...
        subm_after_covid = subm.query('condition_end_date > "2020-03-01"')
        #subm_before_covid = subm.query('condition_end_date <= 2020-03-01')
...
for i in observation_feature_concept_ids:
        ...
        #subm_after_covid = subm.query('observation_date > 2020-03-01')
...
for i in device_exposure_feature_concept_ids:
        ...
        subm_after_covid = subm.query('device_exposure_end_date > "2020-03-01"')
        #subm_before_covid = subm.query('device_exposure_end_date <= 2020-03-01')
...
for i in drug_exposure_feature_concept_ids:
        ...
        subm_after_covid = subm.query('drug_exposure_end_date > "2020-03-01"')
        #subm_before_covid = subm.query('drug_exposure_end_date <= 2020-03-01')
...
for i in procedure_feature_concept_ids:
        ...
        subm_after_covid = subm.query('procedure_date > "2020-03-01"')
...
for i in visit_feature_concept_ids:
        ...
        subm_after_covid = subm.query('visit_end_date > "2020-03-01"')
...
rfecv = RFECV(estimator=lr, step=5, cv=StratifiedKFold(2), scoring=auprc_score)

X_remain, X_opt, Y_remain, Y_opt = train_test_split(X, Y, test_size=0.25, stratify=Y)

selector = rfecv.fit(X_opt, Y_opt)
X_selected = selector.transform(X)

selected_feature_indices = selector.get_support(indices=True)
scalerfile = '../model/feature_selector.sav'
...
lr = LogisticRegression(solver='saga').fit(X_selected, Y)

dump(lr, '../model/baseline.joblib')
