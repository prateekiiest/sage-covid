...
def feature_selection_measurement_auprc(X, Y):
	...
	n_features_selected_list = []
		...
		n_features_selected_list += [n_features_selected]
		auprc_scores += [ auprc ]
	...
	n_features_selected_list = np.array(n_features_selected_list)
	auprc_score_flags = auprc_scores >= max_auprc - 0.005
	
	for i in range(len(auprc_scores)):
		auprc_score = auprc_scores[-1-i]
		auprc_score_flag = auprc_score_flags[-1-i]
		n_features_selected = n_features_selected_list[-1-i]
		if (auprc_score_flag == 'True') and (n_features_selected >= best_n_features_selected):
			best_n_features_selected = n_features_selected
