...
def save_dataset_to_arff_file(X, y, arff_filename):

	n_samples = X.shape[0]
	n_features = X.shape[1]

	arff_file = open(arff_filename, 'w')
	arff_file.write('@relation data_set\n')
	...
	for i in range(n_samples):
		...
		for j in range(n_features):
			...
			arff_file.write('{:.5f},'.format(X[i][j]))
		arff_file.write('{}\n'.format(class_label))
	arff_file.close()
...
if ('3003694' in measurement_feature_concept_ids):
        ...
        ordinal_encoder.fit(X_min[:, index_f].reshape(-1,1))
        X_min[:, index_f] = ordinal_encoder.transform(X_min[:, index_f].reshape(-1,1)).reshape(-1)
        ordinal_encoder.fit(X_max[:, index_f].reshape(-1,1))
        X_max[:, index_f] = ordinal_encoder.transform(X_max[:, index_f].reshape(-1,1)).reshape(-1)
        ordinal_encoder.fit(X_ave[:, index_f].reshape(-1,1))
        X_ave[:, index_f] = ordinal_encoder.transform(X_ave[:, index_f].reshape(-1,1)).reshape(-1)
...
#X = np.concatenate((X_selected, X_gender_race_ethnicity), axis=1)
#dataset = np.concatenate((X, Y), axis=1)
X_remain, X_opt, Y_remain, Y_opt = train_test_split(X_selected, Y, test_size=0.25, stratify=Y)
...
arff_filename = "../model/train_opt.arff"
npy_filename = "../model/train_wo_gender_race_ethnicity.npy"
npy_filename_2 = "../model/train_gender_race_ethnicity.npy"
npy_filename_3 = "../model/class_labels.npy"

save_dataset_to_arff_file(X_opt, Y_opt, arff_filename)

npy_file = open(npy_filename, 'wb')
npy_file_2 = open(npy_filename_2, 'wb')
npy_file_3 = open(npy_filename_3, 'wb')
np.save(npy_file, X_selected)
np.save(npy_file_2, X_gender_race_ethnicity)
np.save(npy_file_3, Y)
npy_file.close()
npy_file_2.close()
npy_file_3.close()
...
dump(ordinal_encoder, '../model/ordinal_encoder')
...
print("Feature extraction for train data has finished")
