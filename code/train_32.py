...
selected_feature_indices = range(X_selected.shape[1])
...
clf = RandomizedSearchCV(lgb_classifier, parameters, random_state=0, n_iter=500, n_jobs=4, verbose=1, scoring=auprc_score, cv=3)
...
lgb_classifier = lgb.LGBMClassifier(objective='binary', n_jobs=4, random_state=0, learning_rate=learning_rate_opt, n_estimators=n_estimators_opt, max_depth=max_depth_opt, num_leaves=num_leaves_opt, colsample_bytree=colsample_bytree_opt).fit(X_selected,Y)
