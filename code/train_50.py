...
def feature_selection_measurement_auprc(X, Y):
	...
	#because this is simple MIL model
		...
		X_count_opt_selected = X_opt[:, 3*n_features:(3*n_features+n_features_selected)]
        	...
        	X_opt_selected = np.concatenate((X_opt_selected, X_ave_opt_selected), axis=1)
                X_opt_selected = np.concatenate((X_opt_selected, X_count_opt_selected), axis=1)
	...
	X_count_selected = X[:, 3*n_features:(3*n_features+best_n_features_selected)]

	X_selected = np.concatenate((X_min_selected, X_max_selected), axis=1)
	X_selected = np.concatenate((X_selected, X_ave_selected), axis=1)
        X_selected = np.concatenate((X_selected, X_count_selected), axis=1)
...
for i in measurement_feature_concept_ids:
        ...
        #subm_before_covid_count = subm_before_covid.groupby('person_id')['value_as_number'].count()
        ...
        for person_id in person_ids_before_covid_but_not_after_covid_set:
                ...
                #X_count[index_p][index_f] = subm_before_covid_count[person_id]
...
X_measurement = np.concatenate((X, X_count), axis=1)
...
clf = GaussianNB().fit(X_selected, Y)
