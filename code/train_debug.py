...
def sort_concept_ids_wrt_frequency_of_positives(df, query_string, person_ids_positives, concept_id_name):
        df_positives_after_covid = df.query(query_string)

        if (df_positives_after_covid.empty):
                df_positives_after_covid = df.query('person_id in @person_ids_positives')

        df_positives_after_covid['counts'] = df_positives_after_covid.groupby(concept_id_name)[concept_id_name].transform('count')
        df_positives_after_covid.sort_values('counts', inplace=True, ascending=False)
        df_positives_after_covid = df_positives_after_covid.drop_duplicates(concept_id_name)
        concept_ids_sorted_wrt_frequency = df_positives_after_covid[concept_id_name].tolist()

        return concept_ids_sorted_wrt_frequency
...
csv_filename = '../../ehr-dream-challenges/examples/covid19-question-1/synthetic_data/training/measurement.csv'
...
query_string = 'measurement_date > "2020-01-01" and person_id in @person_ids_positives'
concept_id_name = 'measurement_concept_id'
measurement_concept_ids_sorted_wrt_frequency = sort_concept_ids_wrt_frequency_of_positives(measurement, query_string, person_ids_positives, concept_id_name)

print(datetime.now())

measurement_feature_concept_ids = measurement_concept_ids_sorted_wrt_frequency
...
X_selected_first_step, best_n_features_selected_measurement = feature_selection_measurement_auprc(X, Y)

print("concatenation of feature matrices")

X_selected = X_selected_first_step
