...
n_genders = categories[0].size
n_races = categories[1].size
n_ethnicities = categories[2].size
n_densities = n_genders * n_races * n_ethnicities
...
n_features_selected_wo_gender_race_ethnicity = X_selected.shape[1]
n_dim_gaussian = n_features_selected_wo_gender_race_ethnicity
mean_vectors_zero = np.zeros((n_densities, n_dim_gaussian))
mean_vectors_one = np.zeros((n_densities, n_dim_gaussian))
covariance_matrices_zero = np.zeros((n_densities*n_dim_gaussian, n_dim_gaussian))
covariance_matrices_one = np.zeros((n_densities*n_dim_gaussian, n_dim_gaussian))

X_gender_race_ethnicity_unique = np.unique(X_gender_race_ethnicity, axis=0)
n_unique_gender_race_ethnicity = X_gender_race_ethnicity_unique.shape[1]
class_priors = np.zeros((n_dim_gaussian, 2))

for i in range(n_unique_gender_race_ethnicity):

	gender_race_ethnicity = X_gender_race_ethnicity_unique[i]
	indices = np.where((X_gender_race_ethnicity == gender_race_ethnicity).all(axis=1))[0]
	gender_binary = gender_race_ethnicity[0:n_genders]
	race_binary = gender_race_ethnicity[n_genders:n_genders+n_races]
	ethnicity_binary = gender_race_ethnicity[n_genders+n_races:]
	gender_index = np.argwhere(gender_binary==1)[0][0]
        race_index = np.argwhere(race_binary==1)[0][0]
        ethnicity_index = np.argwhere(ethnicity_binary==1)[0][0]
	density_index = n_races*n_ethnicities*gender_index + n_ethnicities*race_index + ethnicity_index
	Y_selected_conditional = Y[indices]
	X_selected_conditional = X_selected[indices, :]	
	indices_zero = (Y_selected_conditional == 0.0)
	X_selected_conditional_zero = X_selected_conditional[indices_zero, :]
        X_selected_conditional_one = X_selected_conditional[np.invert(indices_zero), :]
	
	mean_zero = np.mean(X_selected_conditional_zero, axis = 0)
	cov_zero = np.cov(X_selected_conditional_zero, rowvar = False)
        mean_one = np.mean(X_selected_conditional_one, axis = 0)
        cov_one = np.cov(X_selected_conditional_one, rowvar = False)	
	mean_vectors_zero[density_index, :] = mean_zero
        mean_vectors_one[density_index, :] = mean_one
	covariance_matrices_zero[density_index*n_dim_gaussian:(density_index+1)*n_dim_gaussian, :] = cov_zero
        covariance_matrices_one[density_index*n_dim_gaussian:(density_index+1)*n_dim_gaussian, :] = cov_one

	count_zero = sum(Y_selected_conditional==0.0)
	count_one = sum(Y_selected_conditional==1.0)

	class_priors[density_index, 0] = count_zero / float(count_zero+count_one)
	class_priors[density_index, 1] = count_one / float(count_zero+count_one)
...
dump(mean_vectors_zero, '../model/mean_vectors_zero')
dump(covariance_matrices_zero, '../model/covariance_matrices_zero')
dump(mean_vectors_one, '../model/mean_vectors_one')
dump(covariance_matrices_one, '../model/covariance_matrices_one')
