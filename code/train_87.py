...
def feature_selection_measurement_auprc(X, Y):
	...
	#auprc_score = make_scorer(compute_auprc, greater_is_better=True)
        ...
        n_samples = X_opt.shape[0]

        n_iterations = n_features
	feature_indices = np.linspace(0, n_features-1, n_features).astype(int)

	auprcs = []

        for i in range(n_iterations):

                X_min_opt_selected = X_opt[:, i].reshape(n_samples,1)
                X_max_opt_selected = X_opt[:, n_features+i].reshape(n_samples,1)
                X_ave_opt_selected = X_opt[:, 2*n_features+i].reshape(n_samples,1)
                ...
                #clf = LogisticRegressionCV(solver='saga', scoring=auprc_score, cv=2)
	...
	auprcs_sorted, feature_indices_sorted = zip( *sorted( zip(auprcs, feature_indices) ) )
	auprcs_sorted = auprcs_sorted[::-1]
	feature_indices_sorted = np.array(feature_indices_sorted[::-1])
        ...
        feature_step_size = 1
	best_feature_indices_selected = feature_indices_sorted[0:feature_step_size]

        if (n_features < feature_step_size):
                feature_step_size = 1
	        best_feature_indices_selected = feature_indices_sorted[0]
		...
		feature_indices_selected = feature_indices_sorted[0:n_features_selected]
                X_min_opt_selected = X_opt[:, feature_indices_selected]
                X_max_opt_selected = X_opt[:, n_features+feature_indices_selected]
                X_ave_opt_selected = X_opt[:, 2*n_features+feature_indices_selected]
                ...
                if (auprc > max_auprc):
                        max_auprc = auprc
			best_feature_indices_selected = feature_indices_selected

        X_min_selected = X[:, best_feature_indices_selected]
        X_max_selected = X[:, n_features+best_feature_indices_selected]
        X_ave_selected = X[:, 2*n_features+best_feature_indices_selected]

        X_selected = np.concatenate((X_min_selected, X_max_selected), axis=1)
        X_selected = np.concatenate((X_selected, X_ave_selected), axis=1)

        return X_selected, best_feature_indices_selected
...
def feature_selection_auprc(X, Y):
        ...
        for i in range(n_iterations):
                ...
                X_opt_selected = X_opt[:, i].reshape(n_samples,1)
	...
	feature_indices_sorted = np.array(feature_indices_sorted[::-1])
        ...
        for i in range(n_iterations):
                ...
                X_opt_selected = X_opt[:, feature_indices_selected]
        ...
        X_selected = X[:, best_feature_indices_selected]
...
#measurement_2 = measurement.loc[(measurement['value_as_concept_id']==45877985.0) | (measurement['value_as_concept_id']==45884084.0)]
#print("measurement RT-PCR shape:")
#print(measurement_2.shape)
...
X_measurement_2 = np.zeros((len(person_index), n_measurement_types))
...
for i in measurement_feature_concept_ids:
        ...
        #subm = measurement_2.query('measurement_date > "2020-01-01" and measurement_concept_id in @i')
        subm = measurement.query('measurement_date > "2020-01-01" and measurement_concept_id in @i')
	...
	if (subm.empty):
        ...
        for person_id in subm['person_id']:
                ...
                X_measurement_2[index_p][index_f] += 1.0
...
X_measurement_selected, best_feature_indices_selected_measurement = feature_selection_measurement_auprc(X_measurement, Y)
X_measurement_selected_2, best_feature_indices_selected_measurement_2 = feature_selection_auprc(X_measurement_2, Y)
X_condition_selected, best_feature_indices_selected_condition = feature_selection_auprc(X_condition, Y)
X_observation_selected, best_feature_indices_selected_observation = feature_selection_auprc(X_observation, Y)
X_device_exposure_selected, best_feature_indices_selected_device_exposure = feature_selection_auprc(X_device_exposure, Y)
X_drug_exposure_selected, best_feature_indices_selected_drug_exposure = feature_selection_auprc(X_drug_exposure, Y)
X_procedure_selected, best_feature_indices_selected_procedure = feature_selection_auprc(X_procedure, Y)
X_visit_selected, best_feature_indices_selected_visit = feature_selection_auprc(X_visit, Y)

print("concatenation of feature matrices")

X_selected = np.concatenate((X_measurement_selected, X_age), axis=1)
X_selected = np.concatenate((X_selected, X_measurement_selected_2), axis=1)
...
dump(best_feature_indices_selected_measurement, '../../model/best_feature_indices_selected_measurement')
dump(best_feature_indices_selected_measurement_2, '../../model/best_feature_indices_selected_measurement_2')
dump(best_feature_indices_selected_condition, '../../model/best_feature_indices_selected_condition')
dump(best_feature_indices_selected_observation, '../../model/best_feature_indices_selected_observation')
dump(best_feature_indices_selected_device_exposure, '../../model/best_feature_indices_selected_device_exposure')
dump(best_feature_indices_selected_drug_exposure, '../../model/best_feature_indices_selected_drug_exposure')
dump(best_feature_indices_selected_procedure, '../../model/best_feature_indices_selected_procedure')
dump(best_feature_indices_selected_visit, '../../model/best_feature_indices_selected_visit')
