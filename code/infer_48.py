...
X_continuous_selected = X_selected[:, is_continuous]
X_discrete_selected = X_selected[:, np.invert(is_continuous)]
...
y_pred_continuous = clf_continuous.predict_proba(X_continuous_selected)
y_pred_discrete = clf_discrete.predict_proba(X_discrete_selected)

y_pred = np.multiply(y_pred_continuous, y_pred_discrete)

y_pred[:, 0] /= class_priors[0]
y_pred[:, 1] /= class_priors[1]

y_pred[:, 0] = np.divide(y_pred[:, 0], (y_pred[:, 0]+y_pred[:, 1]))
y_pred[:, 1] = np.divide(y_pred[:, 1], (y_pred[:, 0]+y_pred[:, 1]))
