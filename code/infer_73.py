...
def compute_log_probability(probability):

        if (probability <= 1e-300):
                log_probability = -1e10
        else:
...
mean_vectors_zero = load('../model/mean_vectors_zero')
covariance_matrices_zero = load('../model/covariance_matrices_zero')
mean_vectors_one = load('../model/mean_vectors_one')
covariance_matrices_one = load('../model/covariance_matrices_one')
class_priors = load('../model/class_priors')
n_dim_gaussian = X_selected.shape[1]

n_densities = class_priors.shape[0]
...
for i in range(len(person_ids)):
        ...
        X_selected_i = X_selected[i, :]
	X_gender_race_ethnicity_i = X_gender_race_ethnicity[i, :]
	gender_binary = X_gender_race_ethnicity_i[0:n_genders]
	race_binary = X_gender_race_ethnicity_i[n_genders:n_genders+n_races]
        ethnicity_binary = X_gender_race_ethnicity_i[n_genders+n_races:]
	...
	mean_zero = mean_vectors_zero[density_index]
        mean_one = mean_vectors_one[density_index]
	cov_zero = covariance_matrices_zero[density_index*n_dim_gaussian:(density_index+1)*n_dim_gaussian, :] + + 0.01*np.identity(n_dim_gaussian)
        cov_one = covariance_matrices_one[density_index*n_dim_gaussian:(density_index+1)*n_dim_gaussian, :] + + 0.01*np.identity(n_dim_gaussian)
        multi_variate_normal_zero = multivariate_normal.pdf(X_selected_i, mean_zero, cov_zero)
        multi_variate_normal_one = multivariate_normal.pdf(X_selected_i, mean_one, cov_one)
        class_prior_zero = class_priors[density_index][0]
        class_prior_one = class_priors[density_index][1]

	log_class_conditional_zero = compute_log_probability(multi_variate_normal_zero)
        log_class_conditional_one = compute_log_probability(multi_variate_normal_one)
	log_class_prior_zero = compute_log_probability(class_prior_zero)
        log_class_prior_one = compute_log_probability(class_prior_one)

        log_x_joint_class_zero = log_class_conditional_zero + log_class_prior_zero
        log_x_joint_class_one = log_class_conditional_one + log_class_prior_one

        numerator = math.exp(log_x_joint_class_one)
        numerator_complement = math.exp(log_x_joint_class_zero)
