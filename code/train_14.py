...
measurement_concept_ids_file = open("../data/measurement_concept_ids.list", "w")
condition_concept_ids_file = open("../data/condition_concept_ids.list", "w")
observation_concept_ids_file = open("../data/observation_concept_ids.list", "w")
device_exposure_concept_ids_file = open("../data/device_exposure_concept_ids.list", "w")
drug_exposure_concept_ids_file = open("../data/drug_exposure_concept_ids.list", "w")
procedure_concept_ids_file = open("../data/procedure_concept_ids.list", "w")
visit_concept_ids_file = open("../data/visit_concept_ids.list", "w")
...
print("find the optimum number of features for measurements")

X_remain, X_measurement_opt, Y_remain, Y_opt = train_test_split(X_measurement, Y, test_size=0.25, stratify=Y)

n_features_x_min = min(100, X_min.shape[1])
feature_step_size = 5
best_n_features_selected = 5
if (n_features_x_min < feature_step_size):
        feature_step_size = 1
        best_n_features_selected = 1
n_iterations = n_features_x_min / feature_step_size
max_auprc = 0
...
for i in range(n_iterations):
        ...
        X_min_opt_selected = X_measurement_opt[:, 0:n_features_selected]
        X_max_opt_selected = X_measurement_opt[:, n_features_x_min:(n_features_x_min+n_features_selected)]
        X_ave_opt_selected = X_measurement_opt[:, 2*n_features_x_min:(2*n_features_x_min+n_features_selected)]

        X_measurement_opt_selected = np.concatenate((X_min_opt_selected, X_max_opt_selected), axis=1)
        X_measurement_opt_selected = np.concatenate((X_measurement_opt_selected, X_ave_opt_selected), axis=1)

        #print(X_measurement_opt_selected.shape)

        clf = LogisticRegression(solver='saga')

        y_pred = cross_val_predict(clf, X_measurement_opt_selected, Y_opt, cv=2, method='predict_proba')
...
print(best_n_features_selected)

X_min_selected = X_measurement[:, 0:best_n_features_selected]
X_max_selected = X_measurement[:, n_features_x_min:(n_features_x_min+best_n_features_selected)]
X_ave_selected = X_measurement[:, 2*n_features_x_min:(2*n_features_x_min+best_n_features_selected)]
...
measurement_feature_concept_ids_selected = measurement_concept_ids_sorted_wrt_frequency[0:best_n_features_selected]
pickle.dump(measurement_feature_concept_ids_selected, measurement_concept_ids_file)

print(datetime.now())

print("find the optimum number of features for conditions")

X_remain, X_condition_opt, Y_remain, Y_opt = train_test_split(X_condition, Y, test_size=0.25, stratify=Y)

n_features_x_condition = min(100, X_condition.shape[1])
feature_step_size = 5
best_n_features_selected = 5
if (n_features_x_condition < feature_step_size):
        feature_step_size = 1
        best_n_features_selected = 1

n_iterations = n_features_x_condition / feature_step_size
max_auprc = 0
...
for i in range(n_iterations):
        ...
        n_features_selected = (i+1)*feature_step_size
        X_condition_opt_selected = X_condition_opt[:, 0:n_features_selected]

        #print(X_condition_opt_selected.shape)

        clf = LogisticRegression(solver='saga')

        y_pred = cross_val_predict(clf, X_condition_opt_selected, Y_opt, cv=2, method='predict_proba')
        precision, recall, thresholds = precision_recall_curve(Y_opt, y_pred[:,1])
        auprc = auc(recall, precision)
...
X_condition_selected = X_condition[:, 0:best_n_features_selected]

condition_feature_concept_ids_selected = condition_concept_ids_sorted_wrt_frequency[0:best_n_features_selected]

pickle.dump(condition_feature_concept_ids_selected, condition_concept_ids_file)

print("find the optimum number of features for observations")

X_remain, X_observation_opt, Y_remain, Y_opt = train_test_split(X_observation, Y, test_size=0.25, stratify=Y)

n_features_x_observation = min(100, X_observation.shape[1])
feature_step_size = 5
best_n_features_selected = 5
if (n_features_x_observation < feature_step_size):
        feature_step_size = 1
        best_n_features_selected = 1

n_iterations = n_features_x_observation / feature_step_size
max_auprc = 0
...
for i in range(n_iterations):
        ...
        n_features_selected = (i+1)*feature_step_size
        X_observation_opt_selected = X_observation_opt[:, 0:n_features_selected]

        #print(X_observation_opt_selected.shape)

        clf = LogisticRegression(solver='saga')

        y_pred = cross_val_predict(clf, X_observation_opt_selected, Y_opt, cv=2, method='predict_proba')
        precision, recall, thresholds = precision_recall_curve(Y_opt, y_pred[:,1])
        auprc = auc(recall, precision)
...
X_observation_selected = X_observation[:, 0:best_n_features_selected]

observation_feature_concept_ids_selected = observation_concept_ids_sorted_wrt_frequency[0:best_n_features_selected]

pickle.dump(observation_feature_concept_ids_selected, observation_concept_ids_file)

print("find the optimum number of features for device_exposures")

X_remain, X_device_exposure_opt, Y_remain, Y_opt = train_test_split(X_device_exposure, Y, test_size=0.25, stratify=Y)

n_features_x_device_exposure = min(100, X_device_exposure.shape[1])
feature_step_size = 5
best_n_features_selected = 5
if (n_features_x_device_exposure < feature_step_size):
        feature_step_size = 1
        best_n_features_selected = 1

n_iterations = n_features_x_device_exposure / feature_step_size
max_auprc = 0
...
for i in range(n_iterations):
        ...
        n_features_selected = (i+1)*feature_step_size
        X_device_exposure_opt_selected = X_device_exposure_opt[:, 0:n_features_selected]

        #print(X_device_exposure_opt_selected.shape)

        clf = LogisticRegression(solver='saga')

        y_pred = cross_val_predict(clf, X_device_exposure_opt_selected, Y_opt, cv=2, method='predict_proba')
        precision, recall, thresholds = precision_recall_curve(Y_opt, y_pred[:,1])
        auprc = auc(recall, precision)
...
X_device_exposure_selected = X_device_exposure[:, 0:best_n_features_selected]

device_exposure_feature_concept_ids_selected = device_concept_ids_sorted_wrt_frequency[0:best_n_features_selected]

pickle.dump(device_exposure_feature_concept_ids_selected, device_exposure_concept_ids_file)

print("find the optimum number of features for drug_exposures")

X_remain, X_drug_exposure_opt, Y_remain, Y_opt = train_test_split(X_drug_exposure, Y, test_size=0.25, stratify=Y)

n_features_x_drug_exposure = min(100, X_drug_exposure.shape[1])
feature_step_size = 5
best_n_features_selected = 5
if (n_features_x_drug_exposure < feature_step_size):
        feature_step_size = 1
        best_n_features_selected = 1

n_iterations = n_features_x_drug_exposure / feature_step_size
max_auprc = 0
...
for i in range(n_iterations):
        ...
        n_features_selected = (i+1)*feature_step_size
        X_drug_exposure_opt_selected = X_drug_exposure_opt[:, 0:n_features_selected]

        #print(X_drug_exposure_opt_selected.shape)

        clf = LogisticRegression(solver='saga')

        y_pred = cross_val_predict(clf, X_drug_exposure_opt_selected, Y_opt, cv=2, method='predict_proba')
        precision, recall, thresholds = precision_recall_curve(Y_opt, y_pred[:,1])
        auprc = auc(recall, precision)
...
X_drug_exposure_selected = X_drug_exposure[:, 0:best_n_features_selected]
drug_exposure_feature_concept_ids_selected = drug_concept_ids_sorted_wrt_frequency[0:best_n_features_selected]

pickle.dump(drug_exposure_feature_concept_ids_selected, drug_exposure_concept_ids_file)

print(datetime.now())

print("find the optimum number of features for procedures")

X_remain, X_procedure_opt, Y_remain, Y_opt = train_test_split(X_procedure, Y, test_size=0.25, stratify=Y)

n_features_x_procedure = min(100, X_procedure.shape[1])
feature_step_size = 5
best_n_features_selected = 5
if (n_features_x_procedure < feature_step_size):
        feature_step_size = 1
        best_n_features_selected = 1

n_iterations = n_features_x_procedure / feature_step_size
max_auprc = 0
...
for i in range(n_iterations):
        ...
        n_features_selected = (i+1)*feature_step_size
        X_procedure_opt_selected = X_procedure_opt[:, 0:n_features_selected]

        #print(X_procedure_opt_selected.shape)

        clf = LogisticRegression(solver='saga')

        y_pred = cross_val_predict(clf, X_procedure_opt_selected, Y_opt, cv=2, method='predict_proba')
        precision, recall, thresholds = precision_recall_curve(Y_opt, y_pred[:,1])
        auprc = auc(recall, precision)
...
X_procedure_selected = X_procedure[:, 0:best_n_features_selected]
procedure_feature_concept_ids_selected = procedure_concept_ids_sorted_wrt_frequency[0:best_n_features_selected]

pickle.dump(procedure_feature_concept_ids_selected, procedure_concept_ids_file)

print(datetime.now())

print("find the optimum number of features for visits")

X_remain, X_visit_opt, Y_remain, Y_opt = train_test_split(X_visit, Y, test_size=0.25, stratify=Y)

#n_features_x_visit = X_visit.shape[1]
n_features_x_visit = 100
feature_step_size = 5
best_n_features_selected = 5
if (n_features_x_visit < feature_step_size):
        feature_step_size = 1
        best_n_features_selected = 1

n_iterations = n_features_x_visit / feature_step_size
max_auprc = 0
...
for i in range(n_iterations):
        ...
        n_features_selected = (i+1)*feature_step_size
        X_visit_opt_selected = X_visit_opt[:, 0:n_features_selected]

        #print(X_visit_opt_selected.shape)

        clf = LogisticRegression(solver='saga')

        y_pred = cross_val_predict(clf, X_visit_opt_selected, Y_opt, cv=2, method='predict_proba')
        precision, recall, thresholds = precision_recall_curve(Y_opt, y_pred[:,1])
        auprc = auc(recall, precision)
...
X_visit_selected = X_visit[:, 0:best_n_features_selected]
visit_feature_concept_ids_selected = visit_concept_ids_sorted_wrt_frequency[0:best_n_features_selected]

pickle.dump(visit_feature_concept_ids_selected, visit_concept_ids_file)
...
scalerfile = '../model/selector.sav'
pickle.dump(selector, open(scalerfile, 'wb'))
...
measurement_concept_ids_file.close()
condition_concept_ids_file.close()
observation_concept_ids_file.close()
device_exposure_concept_ids_file.close()
drug_exposure_concept_ids_file.close()
procedure_concept_ids_file.close()
visit_concept_ids_file.close()
