...
def feature_selection_measurement_auprc(X, Y):
		...
		clf = MLPClassifier(learning_rate_init=0.01, hidden_layer_sizes=(10,))
...
measurement_positives_after_covid = measurement.query('measurement_date > "2019-11-17" and person_id in @person_ids_positives and value_as_number != -10')
measurement_concept_ids_positives_after_covid = measurement_positives_after_covid['measurement_concept_id'].tolist()

measurement_negatives_after_covid = measurement.query('measurement_date > "2019-11-17" and person_id in @person_ids_negatives and measurement_concept_id in @measurement_concept_ids_positives_after_covid and value_as_number != -10')

measurement_positives_after_covid['counts_positives'] = measurement_positives_after_covid.groupby('measurement_concept_id')['measurement_concept_id'].transform('count')
measurement_negatives_after_covid['counts_negatives'] = measurement_negatives_after_covid.groupby('measurement_concept_id')['measurement_concept_id'].transform('count')

measurement_positives_after_covid = measurement_positives_after_covid.drop_duplicates('measurement_concept_id').sort_values(['measurement_concept_id'])
measurement_negatives_after_covid = measurement_negatives_after_covid.drop_duplicates('measurement_concept_id').sort_values(['measurement_concept_id'])
measurement_after_covid = pd.merge(measurement_positives_after_covid, measurement_negatives_after_covid, on = 'measurement_concept_id')
measurement_positives_after_covid_counts = measurement_after_covid['counts_positives'].to_numpy()
measurement_negatives_after_covid_counts = measurement_after_covid['counts_negatives'].to_numpy()

measurement_concept_ids_after_covid = measurement_positives_after_covid['measurement_concept_id'].to_numpy()
ratios = np.true_divide(measurement_positives_after_covid_counts, (1+measurement_negatives_after_covid_counts))

ratios_sorted, concept_ids_sorted = zip( *sorted( zip(ratios, measurement_concept_ids_after_covid) ) )
ratios_sorted = ratios_sorted[::-1]
concept_ids_sorted = concept_ids_sorted[::-1]

measurement_concept_ids_sorted_wrt_ratios = concept_ids_sorted
...
condition_concept_ids_positives_after_covid = condition_positives_after_covid['condition_concept_id'].tolist()

condition_negatives_after_covid = condition.query('condition_end_date > "2019-11-17" and person_id in @person_ids_negatives and condition_concept_id in @condition_concept_ids_positives_after_covid')

condition_positives_after_covid['counts_positives'] = condition_positives_after_covid.groupby('condition_concept_id')['condition_concept_id'].transform('count')
condition_negatives_after_covid['counts_negatives'] = condition_negatives_after_covid.groupby('condition_concept_id')['condition_concept_id'].transform('count')

condition_positives_after_covid = condition_positives_after_covid.drop_duplicates('condition_concept_id').sort_values(['condition_concept_id'])
condition_negatives_after_covid = condition_negatives_after_covid.drop_duplicates('condition_concept_id').sort_values(['condition_concept_id'])
condition_after_covid = pd.merge(condition_positives_after_covid, condition_negatives_after_covid, on = 'condition_concept_id')
condition_positives_after_covid_counts = condition_after_covid['counts_positives'].to_numpy()
condition_negatives_after_covid_counts = condition_after_covid['counts_negatives'].to_numpy()

condition_concept_ids_after_covid = condition_positives_after_covid['condition_concept_id'].to_numpy()
ratios = np.true_divide(condition_positives_after_covid_counts, (1+condition_negatives_after_covid_counts))

ratios_sorted, concept_ids_sorted = zip( *sorted( zip(ratios, condition_concept_ids_after_covid) ) )
ratios_sorted = ratios_sorted[::-1]
concept_ids_sorted = concept_ids_sorted[::-1]

condition_concept_ids_sorted_wrt_ratios = concept_ids_sorted
...
observation_concept_ids_positives_after_covid = observation_positives_after_covid['observation_concept_id'].tolist()

observation_negatives_after_covid = observation.query('observation_date > "2019-11-17" and person_id in @person_ids_negatives and observation_concept_id in @observation_concept_ids_positives_after_covid')

observation_positives_after_covid['counts_positives'] = observation_positives_after_covid.groupby('observation_concept_id')['observation_concept_id'].transform('count')
observation_negatives_after_covid['counts_negatives'] = observation_negatives_after_covid.groupby('observation_concept_id')['observation_concept_id'].transform('count')

observation_positives_after_covid = observation_positives_after_covid.drop_duplicates('observation_concept_id').sort_values(['observation_concept_id'])
observation_negatives_after_covid = observation_negatives_after_covid.drop_duplicates('observation_concept_id').sort_values(['observation_concept_id'])
observation_after_covid = pd.merge(observation_positives_after_covid, observation_negatives_after_covid, on = 'observation_concept_id')
observation_positives_after_covid_counts = observation_after_covid['counts_positives'].to_numpy()
observation_negatives_after_covid_counts = observation_after_covid['counts_negatives'].to_numpy()

observation_concept_ids_after_covid = observation_positives_after_covid['observation_concept_id'].to_numpy()
ratios = np.true_divide(observation_positives_after_covid_counts, (1+observation_negatives_after_covid_counts))

ratios_sorted, concept_ids_sorted = zip( *sorted( zip(ratios, observation_concept_ids_after_covid) ) )
ratios_sorted = ratios_sorted[::-1]
concept_ids_sorted = concept_ids_sorted[::-1]

observation_concept_ids_sorted_wrt_ratios = concept_ids_sorted
...
device_concept_ids_positives_after_covid = device_exposure_positives_after_covid['device_concept_id'].tolist()

device_exposure_negatives_after_covid = device_exposure.query('device_exposure_end_date > "2019-11-17" and person_id in @person_ids_negatives and device_concept_id in @device_concept_ids_positives_after_covid')

device_exposure_positives_after_covid['counts_positives'] = device_exposure_positives_after_covid.groupby('device_concept_id')['device_concept_id'].transform('count')
device_exposure_negatives_after_covid['counts_negatives'] = device_exposure_negatives_after_covid.groupby('device_concept_id')['device_concept_id'].transform('count')

device_exposure_positives_after_covid = device_exposure_positives_after_covid.drop_duplicates('device_concept_id').sort_values(['device_concept_id'])
device_exposure_negatives_after_covid = device_exposure_negatives_after_covid.drop_duplicates('device_concept_id').sort_values(['device_concept_id'])
device_exposure_after_covid = pd.merge(device_exposure_positives_after_covid, device_exposure_negatives_after_covid, on = 'device_concept_id')
device_exposure_positives_after_covid_counts = device_exposure_after_covid['counts_positives'].to_numpy()
device_exposure_negatives_after_covid_counts = device_exposure_after_covid['counts_negatives'].to_numpy()

device_concept_ids_after_covid = device_exposure_positives_after_covid['device_concept_id'].to_numpy()
ratios = np.true_divide(device_exposure_positives_after_covid_counts, (1+device_exposure_negatives_after_covid_counts))

ratios_sorted, concept_ids_sorted = zip( *sorted( zip(ratios, device_concept_ids_after_covid) ) )
ratios_sorted = ratios_sorted[::-1]
concept_ids_sorted = concept_ids_sorted[::-1]

device_concept_ids_sorted_wrt_ratios = concept_ids_sorted
...
drug_concept_ids_positives_after_covid = drug_exposure_positives_after_covid['drug_concept_id'].tolist()

drug_exposure_negatives_after_covid = drug_exposure.query('drug_exposure_end_date > "2019-11-17" and person_id in @person_ids_negatives and drug_concept_id in @drug_concept_ids_positives_after_covid')

drug_exposure_positives_after_covid['counts_positives'] = drug_exposure_positives_after_covid.groupby('drug_concept_id')['drug_concept_id'].transform('count')
drug_exposure_negatives_after_covid['counts_negatives'] = drug_exposure_negatives_after_covid.groupby('drug_concept_id')['drug_concept_id'].transform('count')

drug_exposure_positives_after_covid = drug_exposure_positives_after_covid.drop_duplicates('drug_concept_id').sort_values(['drug_concept_id'])
drug_exposure_negatives_after_covid = drug_exposure_negatives_after_covid.drop_duplicates('drug_concept_id').sort_values(['drug_concept_id'])
drug_exposure_after_covid = pd.merge(drug_exposure_positives_after_covid, drug_exposure_negatives_after_covid, on = 'drug_concept_id')
drug_exposure_positives_after_covid_counts = drug_exposure_after_covid['counts_positives'].to_numpy()
drug_exposure_negatives_after_covid_counts = drug_exposure_after_covid['counts_negatives'].to_numpy()

drug_concept_ids_after_covid = drug_exposure_positives_after_covid['drug_concept_id'].to_numpy()
ratios = np.true_divide(drug_exposure_positives_after_covid_counts, (1+drug_exposure_negatives_after_covid_counts))

ratios_sorted, concept_ids_sorted = zip( *sorted( zip(ratios, drug_concept_ids_after_covid) ) )
ratios_sorted = ratios_sorted[::-1]
concept_ids_sorted = concept_ids_sorted[::-1]

drug_concept_ids_sorted_wrt_ratios = concept_ids_sorted
...
procedure_positives_after_covid['counts_positives'] = procedure_positives_after_covid.groupby('procedure_concept_id')['procedure_concept_id'].transform('count')
procedure_negatives_after_covid['counts_negatives'] = procedure_negatives_after_covid.groupby('procedure_concept_id')['procedure_concept_id'].transform('count')
...
procedure_positives_after_covid_counts = procedure_after_covid['counts_positives'].to_numpy()
procedure_negatives_after_covid_counts = procedure_after_covid['counts_negatives'].to_numpy()
...
visit_positives_after_covid['counts_positives'] = visit_positives_after_covid.groupby('visit_concept_id')['visit_concept_id'].transform('count')
visit_negatives_after_covid['counts_negatives'] = visit_negatives_after_covid.groupby('visit_concept_id')['visit_concept_id'].transform('count')
...
visit_after_covid = pd.merge(visit_positives_after_covid, visit_negatives_after_covid, on = 'visit_concept_id')
visit_positives_after_covid_counts = visit_after_covid['counts_positives'].to_numpy()
visit_negatives_after_covid_counts = visit_after_covid['counts_negatives'].to_numpy()
