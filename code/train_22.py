...
def feature_selection(X, Y):
	...
	for i in range(n_features):
		...
		correlations[i] = np.correlate(X[:, i], Y-0.5)

	#correlations_sorted = np.sort(correlations)
	idx = np.argsort(correlations)
	idx = idx[::-1]
	...
	idx = idx[0:best_n_features_selected]
...
condition_feature_concept_ids = condition_concept_ids_sorted_wrt_frequency[0:200]
...
drug_exposure_feature_concept_ids = drug_concept_ids_sorted_wrt_frequency[0:200]
...
X_measurement_selected, idx_measurement = feature_selection(X_measurement, Y)
X_condition_selected, idx_condition = feature_selection(X_condition, Y)
X_observation_selected, idx_observation = feature_selection(X_observation, Y)
X_device_exposure_selected, idx_device_exposure = feature_selection(X_device_exposure, Y)
X_drug_exposure_selected, idx_drug_exposure = feature_selection(X_drug_exposure, Y)
X_procedure_selected, idx_procedure = feature_selection(X_procedure, Y)
X_visit_selected, idx_visit = feature_selection(X_visit, Y)
...
measurement_feature_concept_ids_selected = [measurement_feature_concept_ids[index%n_measurement_types] for index in idx_measurement.tolist()]
condition_feature_concept_ids_selected = [condition_feature_concept_ids[index] for index in idx_condition.tolist()]
observation_feature_concept_ids_selected = [observation_feature_concept_ids[index] for index in idx_observation.tolist()]
device_exposure_feature_concept_ids_selected = [device_exposure_feature_concept_ids[index] for index in idx_device_exposure.tolist()]
drug_exposure_feature_concept_ids_selected = [drug_exposure_feature_concept_ids[index] for index in idx_drug_exposure.tolist()]
procedure_feature_concept_ids_selected = [procedure_feature_concept_ids[index] for index in idx_procedure.tolist()]
visit_feature_concept_ids_selected = [procedure_feature_concept_ids[index] for index in idx_visit.tolist()]
...
dump(idx_measurement, '../model/feature_indices_measurement')
dump(idx_condition, '../model/feature_indices_condition')
dump(idx_observation, '../model/feature_indices_observation')
dump(idx_device_exposure, '../model/feature_indices_device_exposure')
dump(idx_drug_exposure, '../model/feature_indices_drug_exposure')
dump(idx_procedure, '../model/feature_indices_procedure')
dump(idx_visit, '../model/feature_indices_visit')
