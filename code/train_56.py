...
location = pd.read_csv('../ehr-dream-challenges/examples/covid19-question-1/synthetic_data/training/location.csv')
...
X_gender_race_ethnicity_location_raw = np.zeros((len(person_index), 4))
...
print("gender, race, ethnicity, location (county)")
...
location_ids = person['location_id'].to_numpy()

for i in range(n_persons):

        person_id = person_ids[i]
        index_p = person_index[person_id]
        X_gender_race_ethnicity_location_raw[index_p][0] = genders[i]
        X_gender_race_ethnicity_location_raw[index_p][1] = races[i]
        X_gender_race_ethnicity_location_raw[index_p][2] = ethnicities[i]

        if (np.isnan(location_ids[i])):
                continue

        df = location[location['location_id'] == location_ids[i]]
        if (df.empty):
                continue
                
        county = df[['county']].values[0][0]
        if (not np.isnan(county)):
                X_gender_race_ethnicity_location_raw[index_p][3] = county

#include_location_flag = 1
#if (not np.any(X_gender_race_ethnicity_location_raw[:,3])):
#	X_gender_race_ethnicity_location_raw = X_gender_race_ethnicity_location_raw[:,0:2]
#	include_location_flag = 0

one_hot_encoder = enc.fit(X_gender_race_ethnicity_location_raw)
X_gender_race_ethnicity_location = enc.transform(X_gender_race_ethnicity_location_raw).toarray()
...
X_selected = np.concatenate((X_selected, X_gender_race_ethnicity_location), axis=1)
