...
ordinal_encoder_filename = "../model/ordinal_encoder"
...
ordinal_encoder = load(open(ordinal_encoder_filename, "rb"))
...
location = pd.read_csv('../ehr-dream-challenges/examples/covid19-question-1/synthetic_data/evaluation/location.csv')
...
X_location_raw = np.zeros((len(person_index), 1))
X_location = np.zeros((len(person_index), 6)) #since there are 39 counties in WA
...
for i in range(n_persons):
        ...
        if (not np.isnan(county)):
                X_location_raw[index_p][0] = county


X_location_ordinal = ordinal_encoder.transform(X_location_raw)
n_cols = X_location.shape[1]

for i in range(n_persons):

        binary = np.array(list(bin(int(X_location_ordinal[i][0]))[2:])).astype(int)

        if (binary.size > 6):
                continue

        col_indices = np.linspace(n_cols-len(binary), n_cols-1, len(binary)).astype(int)
        X_location[i][col_indices] = binary
...
X_selected = np.concatenate((X_selected, X_location), axis=1)
