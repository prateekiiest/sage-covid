...
#clf = GridSearchCV(gbm, parameters, n_jobs=4, verbose=1, scoring=auprc_score, cv=3)
clf = RandomizedSearchCV(gbm, parameters, random_state=0, n_iter=100, n_jobs=4, verbose=1, scoring=auprc_score, cv=3)

search = clf.fit(X_opt, Y_opt)
best_params = search.best_params_
learning_rate_opt = best_params['learning_rate']
n_estimators_opt = best_params['n_estimators']
subsample_opt = best_params['subsample']
max_depth_opt = best_params['max_depth']
max_features_opt = best_params['max_features']

print(datetime.now())
print("model training")

gbm = GradientBoostingClassifier(learning_rate=learning_rate_opt, n_estimators=n_estimators_opt, subsample=subsample_opt, max_depth=max_depth_opt, max_features=max_features_opt).fit(X_selected, Y)

print(datetime.now())

dump(gbm, '../model/baseline.joblib')
