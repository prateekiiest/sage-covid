...
dd = pd.read_csv('../data/data_dictionary.csv', usecols = ['concept_id', 'concept_name'])

pa_mesh = pd.read_csv('../data/xmlmesh/pa2020.csv')
pa_atc = pd.read_csv('../data/ATC.csv')
...
dd_query_result = dd.query('concept_id in @drug_concept_ids_sorted_wrt_frequency')

non_empty_count = 0
empty_count = 0

drug_concept_ids_pas = []

for index, row in dd_query_result.iterrows():

	drug_concept_id = row['concept_id']
	drug_concept_name = row['concept_name']

	drug_concept_name_sub = drug_concept_name.split("MG")[0]
	drug_concept_name_sub = drug_concept_name_sub.split('(Inactivated)')[0]
	drug_concept_name_sub = drug_concept_name_sub.split(',')[0]
	drug_concept_name_sub = drug_concept_name_sub.split('/')[0]
	drug_concept_name_sub = drug_concept_name_sub.split('Oral')[0]
	
	token_list = drug_concept_name_sub.split()
	drug_name = ''
	for i in range(len(token_list)):
		string = token_list[i]
		if (string[0] == '{') or ((string[0] == '(') and (string[-1] == ')')):
			continue
		...
		if (string.isupper() and (len(string) <= 2)):
			continue

		drug_name += string + ' '

	drug_name = drug_name.rstrip().lower()
	if (len(drug_name) == 0):
		print(drug_concept_name)
		continue

	#print(drug_name)

	
	pa_mesh_sub = pa_mesh[pa_mesh["RecordName"].str.contains(drug_name, case=False)]

	words = drug_name.split()

	#pa_sub = pa[pa["text"].apply(lambda sentence: all(word in sentence for word in words))]
	masks = (pa_atc['text'].str.contains(string, case=False) for string in words)
	combined_mask = np.vstack(masks).all(axis=0)
	pa_atc_sub = pa_atc[combined_mask]

	#print(pa_sub)

	#pa_sub = pa[pa["text"].str.contains(pat, case=False)]

	if (pa_atc_sub.empty and pa_mesh_sub.empty):
		empty_count += 1
		drug_concept_ids_pas.append([drug_concept_id, drug_name])
	else:
		if (not pa_atc_sub.empty):

			pas_list = pa_atc_sub['text'].tolist()
			for i in range(len(pas_list)):
				drug_concept_ids_pas.append([drug_concept_id, pas_list[i]])
		else:
                        pas_list = pa_mesh_sub['DescriptorName'].tolist()
                        for i in range(len(pas_list)):
				drug_concept_ids_pas.append([drug_concept_id, pas_list[i]])
...
#print(drug_concept_ids_pas)

drug_concept_ids_pas_df = pd.DataFrame(drug_concept_ids_pas, columns=['drug_concept_id', 'PA'])
drug_concept_ids_pas_df.to_csv('../data/drug_concept_ids_pas.csv')

drug_exposure_feature_concept_ids = drug_concept_ids_sorted_wrt_frequency[0:100]
pas_list_unique = drug_concept_ids_pas_df.query('drug_concept_id in @drug_exposure_feature_concept_ids').drop_duplicates(subset=['PA'])['PA'].tolist()

print("Number of pharmaceutical activities for selected drugs (1st 100):")
print(len(pas_list_unique))
...
drug_exposure_feature_index = dict(zip(pas_list_unique, range(len(pas_list_unique))))
...
n_drug_exposure_types = len(pas_list_unique)
...
for i in drug_exposure_feature_concept_ids:
	...
	pas = drug_concept_ids_pas_df.query('drug_concept_id in @i')['PA'].tolist()
		...
		for j in range(len(pas)):
			...
			index_f = drug_exposure_feature_index[pas[j]]
           		index_p = person_index[person_id]
